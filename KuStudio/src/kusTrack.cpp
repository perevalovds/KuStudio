...
//---------------------------------------------------------------------
void kusTrack::shift_time_sec(float sec, bool only_fragment ) {   //сдвиг влево на заданное число секунд
	shift_time_frames(sec * rate_, only_fragment);
}

//---------------------------------------------------------------------
void kusTrack::shift_time_frames(int k, bool only_fragment) {   //сдвиг влево на заданное число кадров
    if ( k == 0 ) return;
	int i0 = 0;
	int i1 = n_;
	if (only_fragment) { //редактируем только выделенный фрагмент
		i0 = int(kus.time0 * rate_);
		i1 = int(kus.time1 * rate_) + 1;
		i0 = min(max(i0, 0), n_ - 1);
		i1 = min(max(i1, 0), n_);
	}
...
//---------------------------------------------------------------------
void kusTrack::uniformIntTrackFragment(int n0) {  //Выровнять события int-трека
	if (type_ != TYPE_INT) return;
	int i0 = int(kus.time0 * rate_);
	int i1 = int(kus.time1 * rate_) + 1;
	i0 = min(max(i0, 0), n_ - 1);
	i1 = min(max(i1, 0), n_);

	//сбор событий в массив
	//событие - это когда 0 переходит в не 0.
	vector<int> v;
	for (int i = i0; i < i1; i++) {
		if (data_[i] > 0 && (i == 0 || (i > 0 && data_[i-1] == 0))) {
			v.push_back(i);
		}
	}
	if (v.size() < 2) return;	//ничего не надо делать, если меньше двух отсчетов
	//взятие расстояния
	int L = v[v.size() - 1] - v[0];
	//новое число событий - если n0 == -1, то исходное количество, иначе - заданное n0
	int n = (n0 >= 0) ? n0 : v.size();
	v.resize(n);
	//устанавливаем отсчеты
	for (int i = 0; i < n; i++) {
		v[i] = v[0] + i * L / (n - 1);
	}
	//вписываем результат в трек
	for (int i = i0; i < i1; i++) {
		data_[i] = 0;
	}
	for (int i = 0; i < n; i++) {
